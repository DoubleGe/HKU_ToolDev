...
public class TileLine : PaintTool
{
    ...
    public override void RunTool()
    {
        ...
        if (startDrag)
        {
            DrawLinePreview(dragStart, tilePosition);
    ...
    private void DrawLinePreview(Vector2Int start, Vector2Int end)
    {
        int dx = Mathf.Abs(end.x - start.x);
        int dy = Mathf.Abs(end.y - start.y);
        int sx = (start.x < end.x) ? 1 : -1;
        ...
        while (true)
        {
            previewLayer.SetPreviewTile(new Vector2Int(x, y), tileData.tile);
